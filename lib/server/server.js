(function() {
  var Message, app, connect, create_color, create_session_token, formidable, fs, io, move_file, move_uploaded_file, redis, redis_client, sio, upload_file, upload_middleware, util;
  ...
  app = connect().use(connect.logger('dev')).use(upload_middleware).use(connect.static('lib/client')).use(connect.static('images')).listen(3001);
  ...
  upload_file = function(req, res) {
    ...
    return form.parse(req, function(error, fields, files) {
      ...
      if (error || !files.image) {
        ...
        file = files.image;
        return move_uploaded_file(file, function(path) {
          res.writeHead(200, {
            'Content-Type': 'application/json'
          });
