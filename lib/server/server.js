(function() {
  var FileReceiver, Message, app, connect, create_color, create_session_token, fs, io, pruning, sio, token_to_socket, upload_middleware, util;
  ...
  upload_middleware = function(req, res, next) {
    ...
    if (req.url === '/upload' && req.method.toLowerCase() === 'post') {
      ...
      receiver.on('progress', function(percent, transferred, total) {
        return socket.emit('upload_progress', {
          percent: percent,
          transferred: transferred,
          total: total
        });
      });
      return receiver.on('start', function() {
        return socket.emit('upload_started');
      });
    } else {
      return next();
    }
  ...
  io.sockets.on('connection', function(socket) {
    socket.on('get_messages', function(params) {
      return Message.all(function(error, messages) {
